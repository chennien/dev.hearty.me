...
function tp_pay(payvia, prime, card_country){ // eg. card_country = TW
	...
	if(prime.length>0){
		...
		if(payvia==1){
			...
			if($phone_no.length>0){
				...
				if(!!phone_cc && phone_no.length>3){
					...
					/* ### debug
					// 測試環境中，覆寫參數：卡別=TW
					if($("body").attr("data-prod")==0 && phone_no==912345678){
						data["phone"] = "0912345678";
						alertify.success("手機設為：0912345678");
					}
					*/
				}
				// 要求補填手機
		...
		// ### debug
		tp_msg("* fn tp_pay: "+JSON.stringify(data));
		// tp_dpay_btn(true); return;
		// ### debug
		...
		$.ajax({
			type: "post", 
			url: location.origin+"/shop/tp.prime.beta?"+tp_params(true), // ### debug
			async: true, 
			crossDomain: true, 
			dataType: "json", 
			data: data, 
